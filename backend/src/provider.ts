...
export async function embed(texts: string[]): Promise<number[][]> {
  ...
  for (let start = 0; start < texts.length; start += EMBED_BATCH_SIZE) {
    ...
    const batchEmbeddings = await withRetries(`Embedding batch ${start / EMBED_BATCH_SIZE + 1}`, async () => {
      const response = await gemini.models.embedContent({
        model: embedModel,
        contents: batch,
      });

      const values = (response.embeddings ?? []).map(
        (entry) => entry.values ?? [],
      );
...
function mockGenerate(question: string, contexts: RetrievedContext[]): string {
  ...
  const ranked = sentences
    .map((sentence) => {
      const overlap = tokenize(sentence).filter((token) =>
